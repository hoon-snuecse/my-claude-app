...
export async function GET(request) {
  try {
    ...
    const transformedPosts = posts.map(post => ({
      ...
      images: post.teaching_post_images ? post.teaching_post_images.map(img => ({
        id: img.id,
        name: img.file_name,
        url: `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/teaching-images/${img.file_path}`
      })) : [],
      files: post.teaching_post_files ? post.teaching_post_files.map(file => ({
        id: file.id,
        name: file.file_name,
        url: `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/teaching-images/${file.file_path}`,
        type: file.mime_type,
        size: file.file_size
      })) : []
    }));
    ...
    console.error('Error in GET /api/teaching/posts:', error);
    return NextResponse.json({ error: 'Failed to load posts' }, { status: 500 });
  }
}
