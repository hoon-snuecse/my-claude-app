...
import { BookOpen, Users, Lightbulb, Plus, Save, X, Loader2, Image as ImageIcon, Upload, FileText, Paperclip, Music, Video } from 'lucide-react';
...
function WritePageContent() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const endpoint = '/api/teaching/posts/supabase';
      ...
      if (response.ok) {
        router.push(`/teaching/${editId || responseData.id}`);
      } else {
      ...
      for (const file of files) {
        ...
        if (error) {
          ...
          if (error.message?.includes('row too large')) {
            ...
            throw new Error('Storage가 올바르게 설정되지 않았습니다. Supabase 대시보드에서 teaching-images 버킷을 확인해주세요.');
          } else {
    ...
    <div className="min-h-screen">
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto p-6">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-3xl font-bold text-slate-800">교육 포스트 작성</h1>
          <Link
            href="/teaching"
        ...
        <div className="mb-6">
          ...
          <label className="flex items-center gap-3 px-4 py-3 border-2 border-dashed border-slate-300 rounded-lg hover:border-blue-400 cursor-pointer transition-colors">
            ...
            <span className="text-slate-600">
              {uploadingFile ? '업로드 중...' : 'PDF, DOC, MP3, MP4 등 파일 선택 (최대 50MB)'}
            </span>
            <input
              type="file"
...
export default function TeachingWritePage() {
  return (
