...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const timers = [
      ...
      setTimeout(() => setFadeIn(prev => ({ ...prev, subtitle: true })), 2500),
      setTimeout(() => setFadeIn(prev => ({ ...prev, description: true })), 3000),
      setTimeout(() => setFadeIn(prev => ({ ...prev, quote1: true })), 3500),
      setTimeout(() => setFadeIn(prev => ({ ...prev, quote2: true })), 3700),
      setTimeout(() => setFadeIn(prev => ({ ...prev, quote3: true })), 3900),
    ];
    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <section className={`py-24 px-4 text-center transition-all duration-1000 ${
          fadeIn.hero ? 'opacity-100' : 'opacity-0'
        }`}>
          <div className="container-custom">
            {/* 타이핑 애니메이션 타이틀 */}
            <TypingAnimation 
              text="BlueNote Atelier"
              className="text-5xl md:text-6xl font-space-grotesk font-bold mb-6"
              delay={300}
            />
            
            <p className={`text-xl md:text-2xl text-slate-600 mb-8 font-medium transition-all duration-1000 ${
              fadeIn.subtitle ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
            }`}>
              미완성된 생각들이 살아 숨쉬는 공간
            </p>
            
            <p className={`text-lg text-slate-500 max-w-3xl mx-auto leading-relaxed transition-all duration-1000 ${
        ...
        {/* Quotes Section with enhanced cards */}
        <section className="py-16 px-4">
          <div className="container-custom space-y-8">
            {quotes.map((quote, index) => (
              <div
                key={index}
                className={`quote-sheet group hover:shadow-xl transition-all duration-1000 transform hover:-translate-y-2 ${
                  fadeIn[`quote${index + 1}`] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
                }`}
              >
                <div className="relative">
                  {/* Enhanced dashed border effect */}
                  <div className="absolute inset-4 border border-dashed border-blue-200 rounded-lg opacity-30 group-hover:opacity-50 transition-opacity"></div>
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="flex items-center gap-2 text-blue-600 font-semibold mb-2">
                      <span className="w-5 h-px bg-blue-300 group-hover:w-10 transition-all"></span>
